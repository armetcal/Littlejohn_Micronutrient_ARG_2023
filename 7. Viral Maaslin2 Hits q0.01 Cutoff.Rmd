...
temp = ps.rel %>% mutate(Genus = factor(Genus, levels = order$Genus)) %>% 
  ...
  scale_y_log10() +
  ggtitle('Virome: q<0.01, MaAsLin2')
ggsave('Plots/Virome/P3_genus_q0.01_pos_virome.png', height = 5, width =7)
...
temp2 = ps.rel %>% mutate(Genus = factor(Genus, levels = order$Genus)) %>% 
  ...
  scale_y_log10() +
  ggtitle('Virome: q<0.01, MaAsLin2')
ggsave('Plots/Virome/P3_genus_q0.01_neg_virome.png', height = 5, width =5)
```

```{r Combined Figure}
g = ggarrange(plotlist = list(temp,temp2) %>% lapply(function(x)x + ggtitle(NULL)),ncol=2,widths = c(6.5,4.5),common.legend = T,legend = 'right')
ggsave('Plots/Virome/P3_genus_maaslin2_q0.01_combined_virome.jpeg', height = 5, width =10)
```
...
```{r}
# Maaslin stats
maas = readxl::read_xlsx('Results Files/Virome/Maaslin2_longitudinal_viruses.xlsx',sheet='Family') %>% filter(qval<0.01)
...
temp = ps.rel %>% mutate(Family = factor(Family, levels = order$Family)) %>% 
  ...
  scale_y_log10() +
  ggtitle('Virome: q<0.01, MaAsLin2')
ggsave('Plots/Virome/P3_family_q0.01_pos_virome.png', height = 5, width =7)
...
temp2 = ps.rel %>% mutate(Family = factor(Family, levels = order$Family)) %>% 
  ...
  scale_y_log10() +
  ggtitle('Virome: q<0.01, MaAsLin2')
ggsave('Plots/Virome/P3_family_q0.01_neg_virome.png', height = 5, width =5)
```

```{r Combined Figure}
g = ggarrange(plotlist = list(temp,temp2) %>% lapply(function(x)x + ggtitle(NULL)),ncol=2,widths = c(6.5,4.5),common.legend = T,legend = 'right')
ggsave('Plots/Virome/P3_family_maaslin2_q0.01_combined_virome.jpeg', height = 5, width =10)
```