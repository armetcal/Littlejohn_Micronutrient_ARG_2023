...
# Maaslin stats
maas = readxl::read_xlsx('Results Files/Virome/Maaslin2_longitudinal_viruses.xlsx',sheet='All Results')
maas$Species[maas$Species=='Deltaentomopoxvirus Melanoplus sanguinipes entomopoxvirus'] = 'Deltaentomopoxvirus Melanoplus\nsanguinipes entomopoxvirus'
...
capture.output(summary(indval),'Results Files/Virome/F2_indicspecies_results_summary_viruses.txt')
...
write.csv(ind, 'Results Files/Virome/P2_indicspecies_unique_results_viruses.csv',row.names=F)
```

# Increased in 1 group only ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
df_ind_s_grp$Species[df_ind_s_grp$Species=='Deltaentomopoxvirus Melanoplus sanguinipes entomopoxvirus'] = 'Deltaentomopoxvirus Melanoplus\nsanguinipes entomopoxvirus'
...
ggsave('Plots/Virome/P2_indicspecies_viruses.png', height = 6, width = 7)
```
...
#   scale_size(range = c(0.5, 12)) + xlab('')
# 
# ggsave('Plots/Virome/P2_indicspecies_viruses_depletedin1.png', height = 5, width = 7)
```

```{r Plot taxa of interest}
psg.init = ps %>% tax_glom('Species') %>% microbiome::transform('compositional') %>% 
  psmelt()
psg.init$Species[psg.init$Species=='Deltaentomopoxvirus Melanoplus sanguinipes entomopoxvirus'] = 'Deltaentomopoxvirus Melanoplus\nsanguinipes entomopoxvirus'
psg.init = psg.init %>% 
...
p
ggsave('Plots/Virome/P2_all_d28_indicator_species_virome.jpeg',height = 6, width = 10.5)
```

```{r Plot taxa of interest}
# df_ind_s_grp$Species[df_ind_s_grp$Species=='Deltaentomopoxvirus Melanoplus\nsanguinipes entomopoxvirus'] = 'Deltaentomopoxvirus Melanoplus sanguinipes entomopoxvirus'
# 
...
#   arrange(-Abundance)
# 
# p1 = psg %>% filter(Indicator == 'CON Day 28') %>% droplevels() %>% 
#   mutate(Species = factor(Species,levels = c(psg$Species[psg$Indicator=='CON Day 28'] %>% unique,order$Species))) %>% 
#   ggplot(aes(Species, Abundance, fill = Group)) +
#   geom_boxplot(outlier.shape = NA) +
#   geom_point(position=position_jitterdodge(jitter.height=0,jitter.width = 0.2)) +
#   theme_classic(base_size = 16) +
#   ggpubr::stat_compare_means(aes(group=Group),size=7, label = 'p.signif')+
#   ylab('Relative Abundance') +
#   xlab(NULL) +
#   scale_y_continuous(expand = expansion(mult = c(0.1, 0.15))) +
#   theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))
# 
# p2 = psg %>% filter(Indicator == 'LM Day 28') %>% droplevels() %>% 
#   mutate(Species = factor(Species,levels = c(psg$Species[psg$Indicator=='CON Day 28'] %>% unique,order$Species))) %>% 
#   ggplot(aes(Species, Abundance, fill = Group)) +
#   geom_boxplot(outlier.shape = NA) +
#   geom_point(position=position_jitterdodge(jitter.height=0,jitter.width = 0.2)) +
...
#   theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))
# 
# p = ggarrange(plotlist = list(p1,p2),common.legend = T,legend = 'right', widths = c(2,5))
# p
# ggsave('Plots/Virome/P2_all_species_virome.jpeg',height = 6, width = 10.5)
```