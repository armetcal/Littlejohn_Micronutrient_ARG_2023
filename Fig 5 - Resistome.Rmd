...
# Combine pathobiont and CARD data
df = df.res %>% left_join(path %>% select(Sample, Day, Diet, Abundance, Pathobiont, cat))
...
annotations1 = temp %>% select(Drug_Class,Pathobiont,Diet,label.lm,label.con) %>% unique 
...
p1 = temp %>%
  ...
  scale_y_log10()
ggsave('Plots/Functional/Fig 5e - ARG Pathobiont Scatterplots.png',height=6,width=20)
...
ggsave('Plots/Functional/Fig. 5f - Total ARG vs Total Pathobiont.png',height = 5, width = 5)
```

```{r Antibiotic efflux vs Enterococcus}
temp = psm %>% filter(Day == 'Day 28') %>% 
  left_join(path) %>% 
  filter(Pathobiont=='Enterococcus',
         Resistance_Mechanism == 'Antibiotic Efflux') %>% 
  group_by(Sample,Diet,Resistance_Mechanism,Pathobiont,Abundance) %>% 
  summarize(Count=sum(Count)) %>% ungroup
