...
lfc = path %>% 
  ...
  left_join(maas2) %>% 
  arrange(lfc) %>% 
  mutate(`Opportunistic\nBacteria`  = factor(`Opportunistic\nBacteria` , levels = .$`Opportunistic\nBacteria` ),
...
stack = path %>% filter(Day == 'Day 28') %>% 
  group_by(Diet,`Opportunistic\nBacteria`) %>% 
  summarise(Abundance = mean(Abundance)) %>% ungroup()
  
p1 = stack %>% 
  ggplot(aes(x = Diet, y = Abundance, fill = `Opportunistic\nBacteria`)) + 
  geom_bar(stat="identity") +
  theme_classic(base_size = 16) + 
  ylab("Rel Abundance (Day 28)") + xlab('')
data_4a = p1$data %>% select(Diet, `Opportunistic\nBacteria`, Abundance)
...
p2 = summed %>%
  ...
  theme_classic(base_size = 16) + 
  ggpubr::stat_compare_means(comparisons = list(c('CON Day 0','LM Day 0'),c('CON Day 0','CON Day 28'),c("LM Day 0",'LM Day 28'),c('CON Day 28','LM Day 28')),size=5)+
  ylab('Total Opportunistic Bacteria (Rel Ab)') +
  xlab('') + theme(legend.position = 'none')
data_4b = p2$data %>% select(Group, Abundance)
ggsave('Plots/Bacteriome/Fig. 4b - Total Opportunistic Bacteria.png',height = 5, width = 7)
```
...
temp_lfc = lfc %>% arrange(-`LM Day 28`) %>% 
  mutate(`Opportunistic\nBacteria`  = factor(`Opportunistic\nBacteria` , levels = .$`Opportunistic\nBacteria` ))
path$`Opportunistic\nBacteria`  = factor(path$`Opportunistic\nBacteria` , levels = temp_lfc$`Opportunistic\nBacteria` )
maas2$`Opportunistic\nBacteria`  = factor(maas2$`Opportunistic\nBacteria` , levels = temp_lfc$`Opportunistic\nBacteria` )

cols = c('CON' = '#FC746E','LM' = '#01BEC3')
max_per_path = path %>% group_by(`Opportunistic\nBacteria` ) %>% summarize(M = 2*max(Abundance)) %>% ungroup %>% 
  mutate(`Opportunistic\nBacteria`  = factor(`Opportunistic\nBacteria` , levels = temp_lfc$`Opportunistic\nBacteria` ))
max_pos = max_per_path %>% filter(`Opportunistic\nBacteria`  %in% temp_lfc$`Opportunistic\nBacteria` [temp_lfc$lfc>0]) %>% droplevels() %>%
  arrange(`Opportunistic\nBacteria` ) %>% mutate(N = c(1:nrow(.))+0.25)

p3p = path %>% left_join(max_pos) %>% 
  left_join(temp_lfc) %>% left_join(maas2) %>% 
  filter(`Opportunistic\nBacteria`  %in% temp_lfc$`Opportunistic\nBacteria` [temp_lfc$lfc>0]) %>% 
  mutate(Abundance = Abundance + min(.$Abundance[.$Abundance !=0])/2) %>% 
  ggplot(aes(`Opportunistic\nBacteria` , Abundance, fill = Diet)) +
   geom_boxplot(outlier.shape = NA) +
  ...
  scale_y_log10()
data_4c = p3p$data %>% select(Diet, `Opportunistic\nBacteria`, Abundance, qval, symb)
ggsave('Plots/Bacteriome/Fig. 4c - Increased Opportunistic Bacteria.png', height = 5, width =9)
```
...
max_per_path = path %>% group_by(`Opportunistic\nBacteria` ) %>% summarize(M = 2*max(Abundance)) %>% ungroup %>% 
  mutate(`Opportunistic\nBacteria`  = factor(`Opportunistic\nBacteria` , levels = temp_lfc$`Opportunistic\nBacteria` ))
max_neg = max_per_path %>% filter(`Opportunistic\nBacteria`  %in% temp_lfc$`Opportunistic\nBacteria` [temp_lfc$lfc<0]) %>% droplevels() %>%
arrange(`Opportunistic\nBacteria` ) %>% mutate(N = c(1:nrow(.))+0.25)
...
p3n = path %>% left_join(max_neg) %>% 
  ...
  geom_text(aes(x=N,y=M,label=symb), size = 6,lineheight=0.8,hjust=1) +
  scale_y_log10()
data_4d = p3n$data %>% select(Diet, `Opportunistic\nBacteria`, Abundance, qval, symb)
ggsave('Plots/Bacteriome/Fig. 4d - Decreased Opportunistic Bacteria.png', height = 5, width =6)
```
...
data_4e = p$data %>% select(Group, Abundance)
...
data_4f = p2$data %>% select(Group, Species, Abundance)
ggsave('Plots/Fungome/Fig. 4f - Candida Species.png',height = 5, width = 14)
```

```{r}
L = list('Fig 4a' = data_4a,
         'Fig 4b' = data_4b, 
         'Fig 4c' = data_4c,
         'Fig 4d' = data_4d,
         'Fig 4e' = data_4e,
         'Fig 4f' = data_4f)
writexl::write_xlsx(L,'Source Data/Source Data Fig 4.xlsx')
```