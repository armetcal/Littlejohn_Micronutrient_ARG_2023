...
df.stats.con = df %>% filter(Diet=='CON',Day=='Day 28') %>% 
  group_by(Resistance_Mechanism,`Opportunistic\nBacteria`,cat) %>% 
...
p=df.stats %>% 
  ...
  mutate(Qval = ifelse(qval>0.05,'',
                      ifelse(qval>0.01,'*',
                             ifelse(qval>0.001,'**',
                                    ifelse(qval<=0.001,'***',''))))) %>% 
  ggplot(aes(Resistance_Mechanism,`Opportunistic\nBacteria`,col = Rho)) +
  geom_point(size =14) +
...
data_5f = p$data %>% select(Group,Resistance_Mechanism,`Opportunistic\nBacteria`,Rho,qval,label.con,label.lm)
ggsave('Plots/Functional/Fig 5f - Opportunistic Bacteria Heatmap.jpeg',height=7.5,width=6)
```

```{r}
df.sig = df.stats %>% filter(qval<0.05) %>% 
  mutate(comparison = paste0(Resistance_Mechanism,`Opportunistic\nBacteria`))
...
temp = df %>%
  ...
  left_join(df.stats %>% select(Resistance_Mechanism,`Opportunistic\nBacteria`,Group,label.lm,label.con) %>% rename(Diet=Group))

annotations = temp %>% select(Resistance_Mechanism,`Opportunistic\nBacteria`,Diet,label.lm,label.con) %>% unique
...
p = temp %>% 
  ...
  xlab('Rel. Ab, Day 28') +
  ylab('RPKM, Day 28') +
  ggh4x::facet_nested(. ~ Resistance_Mechanism + `Opportunistic\nBacteria`,
                      scales = 'free',space='free_y',axes='margins') +
  theme(legend.position = 'none') +
  scale_y_log10()
data_5g = p$data %>% select(Diet,Resistance_Mechanism,`Opportunistic\nBacteria`,Abundance,Count) %>% 
  rename(mechanims_count = Count, bacterial_abundance = Abundance)
ggsave('Plots/Functional/Fig 5g - Opportunistic Bacteria CON.jpeg',height=4,width=4)
```

```{r Save}
L = list('Fig 5f' = data_5f,
         'Fig 5g' = data_5g)
writexl::write_xlsx(L,'Source Data/Source Data Fig 5fg.xlsx')
```
...
# Fig. 5d - Correlations with `Opportunistic\nBacteria`s
...
p1 = temp %>%
  ...
  scale_y_log10()
data_5d = p1$data %>% select(Diet,Drug_Class,Count,`Opportunistic\nBacteria`,Abundance) %>% 
  rename(drugclass_count = Count, bacterial_abundance = Abundance)
